...
tidb_8_5() {
    ...
    $CONTAINER_CLI run --name tidb -p4000:4000 -d --network tidb_network ${DB_IMAGE_TIDB_8_5:-docker.io/pingcap/tidb:v8.5.4}
    # Give the container some time to start
    OUTPUT=
    n=0
    until [ "$n" -ge 5 ]
    ...
    $CONTAINER_CLI run -it --rm --network tidb_network docker.io/mysql:8.4 mysql -htidb -P4000 -uroot -e "create database hibernate_orm_test; create user 'hibernate_orm_test' identified by 'hibernate_orm_test'; grant all on hibernate_orm_test.* to 'hibernate_orm_test';"
    if [ "$n" -ge 5 ]; then
